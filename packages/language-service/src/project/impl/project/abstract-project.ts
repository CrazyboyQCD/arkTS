...
export abstract class OpenHarmonyProjectImpl implements OpenHarmonyProject {
  ...
  async readOhPackageJson5(): Promise<DeepPartial<OhPackageJson5> | null> {
    ...
    try {
      if (!fs.existsSync(ohPackageJson5Path.fsPath) || !fs.statSync(ohPackageJson5Path.fsPath).isFile())
        return null
      ...
      const ohPackageJson5 = fs.readFileSync(ohPackageJson5Path.fsPath, 'utf-8')
  ...
  async readBuildProfileJson5(): Promise<DeepPartial<BuildProfile> | null> {
    ...
    try {
      if (!fs.existsSync(buildProfileJson5Path.fsPath) || !fs.statSync(buildProfileJson5Path.fsPath).isFile())
        return null
      this.getProjectDetector()
        .getLogger('ProjectDetector/Project/readBuildProfileJson5')
        .getConsola()
        .info(`Read build-profile.json5: ${buildProfileJson5Path.toString()}`)
      const buildProfileJson5 = fs.readFileSync(buildProfileJson5Path.fsPath, 'utf-8')
